...
const AddList = ({ sidebarOpen }) => {
    ...
    try {
      await Promise.all(
        selectedAssets.map((id) =>
          fetch(`https://itassetmgt-7659d44ac774.herokuapp.com/api/v1/asset/${id}`, { method: "DELETE" })
        )
      );
  ...
  const handleToggle = async (id, newStatus) => {
    try {
      await fetch(`https://itassetmgt-7659d44ac774.herokuapp.com/api/v1/asset/${id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://itassetmgt-7659d44ac774.herokuapp.com/api/v1/asset/${id}`, { method: 'DELETE' });
