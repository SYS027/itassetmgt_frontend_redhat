...
const Location = ({ sidebarOpen }) => {
  ...
  const handleDelete = async (itemId) => {
    try {
      const response = await fetch(
        `http://103.228.83.115:8081/api/v1/locations/${itemId}`,
        {
          method: "DELETE",
        }
      );
  ...
  useEffect(() => {
    if (data.state_id) {
      const stateUrl = `http://103.228.83.115:8081/api/v1/states/${data.state_id}`;
      fetch(stateUrl)
        .then((response) => {
          if (response.ok) {
  ...
  useEffect(() => {
    if (data.city_id) {
      const cityUrl = `http://103.228.83.115:8081/api/v1/cities/${data.city_id}`;
      fetch(cityUrl)
        .then((response) => {
          if (response.ok) {
            return response.json();
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const response = await fetch(
        `http://103.228.83.115:8081/api/v1/locations/${id}`,
        {
          method: "PUT",
          headers: {
