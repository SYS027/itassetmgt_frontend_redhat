...
const Product = ({ sidebarOpen }) => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("http://103.228.83.115:8081/api/v1/products");
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `hhttp://103.228.83.115:8081/api/v1/product/destroy/${id}`,
        {
          method: "DELETE",
        }
      );
    ...
    try {
      await Promise.all(
        selectedProducts.map((id) =>
          fetch(`http://103.228.83.115:8081/api/v1/product/destroy/${id}`, {
            method: "DELETE",
          })
        )
      );
  ...
  const handleToggle = async (id, newStatus) => {
    try {
      const response = await fetch(
        `http://103.228.83.115:8081/api/v1/product/${id}`,
        {
          method: "PUT",
          headers: {
