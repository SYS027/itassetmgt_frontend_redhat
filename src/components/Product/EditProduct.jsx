...
const EditProduct = ({ sidebarOpen }) => {
  ...
  useEffect(() => {
    const fetchProductTypes = async () => {
      try {
        const response = await fetch('https://itassetmgt-7659d44ac774.herokuapp.com/api/v1/producttype');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchProductCategories = async () => {
      try {
        const response = await fetch('https://itassetmgt-7659d44ac774.herokuapp.com/api/v1/productcategories');
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
  ...
  const fetchProductsbyid = async () => {
    try {
      const response = await fetch(`https://itassetmgt-7659d44ac774.herokuapp.com/api/v1/products/${params.id}`);
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
      const data = await response.json();
 ...
 const updateProduct = async (e) => {
  e.preventDefault();
  try {
    const response = await fetch(`https://itassetmgt-7659d44ac774.herokuapp.com/api/v1/product/${params.id}`, {
      method: "PUT",
      headers: {
